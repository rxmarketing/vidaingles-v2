<?php
	...
	$sql = "SELECT * FROM paginas WHERE page_published = '1' ORDER BY pageid DESC";
	$qry = mysqli_query($db_conx,$sql);
	while($row = mysqli_fetch_array($qry,MYSQLI_ASSOC)){
    // Set DB variables into local variables for easier use 
    $pageId = $row["pageid"]; 
    $pageFriendlyName = $row["friendlytitle"]; 
    $pagedatepublished = $row["datepublished"]; 
    $pagedatemodified = $row["datemodified"]; 
    ...
    $xmlBody .= '<url>
									<loc>http://vidaingles.com/paginas/'.$pageId.'/'.$pageFriendlyName.'</loc>
									<lastmod>'.$pagedatemodified.'</lastmod>
